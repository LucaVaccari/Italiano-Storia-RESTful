    ...
    public static Integer[] getAuthorIds(HashMap<String, String> filterMap) throws SQLException {
        ...
        for (var entry : filterMap.entrySet()) {
            switch (entry.getKey().toLowerCase()) {
                ...
                case "lifeyear" -> filterArr.add(new BetweenFilter(entry.getValue(),
                        "EXTRACT(YEAR FROM autori.data_nascita)",
                        "EXTRACT(YEAR FROM autori.data_morte)"));
            }
        }
        ...
        while (result.next()) ids.add(result.getInt("id_autore"));
    ...
    public static Integer[] getTopicIds(HashMap<String, String> filterMap) throws SQLException {
        ...
        for (var entry : filterMap.entrySet()) {
            switch (entry.getKey().toLowerCase()) {
                ...
                case "year" -> filterArr.add(new BetweenFilter(entry.getValue(),
                        "EXTRACT(YEAR FROM argomenti.data_inizio)",
                        "EXTRACT(YEAR FROM argomenti.data_fine)"));
            }
        }
        ...
        // execute query and process results
        ResultSet result = sqlConnection.createStatement().executeQuery(sql);
        while (result.next()) ids.add(result.getInt("id_argomento"));
        return ids.toArray(new Integer[0]);
    }
